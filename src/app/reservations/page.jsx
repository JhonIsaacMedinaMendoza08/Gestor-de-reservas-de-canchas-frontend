        ...
        <div className="grid gap-6">
            <div className="bg-card rounded-2xl p-4 shadow-xl">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <div>
                        <label className="text-sm text-muted block mb-1">Fecha (YYYY-MM-DD)</label>
                        <input className="w-full px-3 py-2 rounded-xl border border-[#2a355e] bg-[#0f1530] text-white"
                            name="date" value={filters.date || ''} onChange={(e) => setFilters(f => ({ ...f, date: e.target.value }))} placeholder="2025-08-20" />
                    </div>
                    <div>
                        <label className="text-sm text-muted block mb-1">Cancha</label>
                        <select className="w-full px-3 py-2 rounded-xl border border-[#2a355e] bg-[#0f1530] text-white"
                            name="courtId" value={filters.courtId || ''} onChange={(e) => setFilters(f => ({ ...f, courtId: e.target.value }))}>
                            <option value="">Todas</option>
                            {courts.map((c) => <option key={c._id} value={c._id}>{c.name}</option>)}
                        </select>
                    </div>
                    <div className="flex gap-2">
                        <button className="px-3 py-2 rounded-xl bg-primary text-white" onClick={load}>Aplicar</button>
